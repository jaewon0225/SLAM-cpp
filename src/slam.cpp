...
Eigen::Matrix3d Pose::toTransformationMatrix() const {
  ...
  transformation << cos(orientation), -sin(orientation), position.x(),
      sin(orientation), cos(orientation), position.y(), 0, 0, 1;
...
Edge::Edge(int from, int to, const Pose &measurement,
...
void PoseGraph::addEdge(const Edge &edge) { edges.push_back(edge); }

void PoseGraph::computeErrorAndJacobians(Eigen::MatrixXd &H,
                                         Eigen::VectorXd &b) {
#if !MULTITHREAD
  for (const Edge &edge : edges) {
    ...
    Eigen::Vector2d dtij;
    dtij << xj.position[0] - xi.position[0], xj.position[1] - xi.position[1];
  ...
  for (const Edge &edge : edges) {
    pool.enqueue([&H, &b, &edge, &mtx, this]() {
      ...
      Pose eij_pose = fromTransformationMatrix(eij);
      Eigen::Vector3d eij_vec;
      eij_vec << eij_pose.position[0], eij_pose.position[1],
          eij_pose.orientation;
